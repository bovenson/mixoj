...
class HtmlOutputer(object):
    ...
    def save_problem_to_db(problem_data, operation=FIRST_CRAW):
        ...
        if operation == UPDATE:
            try:
                problems = Problem.objects.filter(ojname=problem_data.get("ojname"),
                                                  sourceid=problem_data.get("sourceid"))
                new_problem = list(problems)[0]
...
if __name__ == "__main__":
    print(len(Problem.objects.filter(ojname="sass")))
